...
export const register = internalAction(async () => {
	...
	if (!token || !convexUrl) {
		throw new Error(
			"TELEGRAM_BOT_TOKEN and CONVEX_SITE_URL must be configured",
		);
	}
	...
	for (const loc of LOCALES) {
		const langTag = loc.language_code
			? ` [${loc.language_code}]`
			: " [default]";
		const langField = loc.language_code
			? { language_code: loc.language_code }
			: {};

		requests.push(
			post("setMyDescription", { description: loc.description, ...langField }),
		);
		...
		requests.push(
			post("setMyCommands", { commands: loc.commands, ...langField }),
		);
	...
	const results = await Promise.all(
		responses.map((res, i) => check(labels[i] ?? "unknown", res)),
	);
